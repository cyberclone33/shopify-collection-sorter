  ...
  if (action === "applyAutomaticDiscounts") {
    try {
      ...
      if (!selectedItemIdsString) {
        return json<ActionData>({
          status: "warning",
          message: "No items selected for automatic discounting",
          discountResult: {
            success: false,
            itemsDiscounted: 0,
            totalItems: 0,
      ...
      console.log(`Applying automatic discounts to ${selectedItemIds.length} selected items`);
      
      // Get selected shelf life items that are synced with Shopify
      const items = await prisma.shelfLifeItem.findMany({
        where: { 
          id: {
            in: selectedItemIds
      ...
      console.log(`Found ${items.length} valid selected items for potential discounting`);
      ...
      for (const item of items) {
        try {
          ...
          if (daysLeft <= 30) {
            ...
            // Medium-high discount for products expiring in 31-60 days
            marginPercentToKeep = 0.35; // Keep 35% of margin
            discountReason = "60_DAYS_LEFT";
            console.log(`Applied 60-day discount tier to item ${item.productId}`);
          } else if (daysLeft <= 90) {
            // Medium discount for products expiring in 61-90 days
            marginPercentToKeep = 0.6; // Keep 60% of margin
            discountReason = "90_DAYS_LEFT";
            console.log(`Applied 90-day discount tier to item ${item.productId}`);
          } else if (daysLeft <= 180) {
            // Light discount for products expiring in 91-180 days
            marginPercentToKeep = 0.8; // Keep 80% of margin
          ...
          console.log(`
            ...
            Margin: ${margin}
            Days Left: ${daysLeft}
            Discount Tier: ${discountReason}
            Keep: ${marginPercentToKeep * 100}% of margin
...
export default function ShelfLifeManagement() {
  ...
  const handleApplyAutomaticDiscounts = () => {
    if (selectedItems.length === 0) {
      ...
      setToastActive(true);
      setToastContent({
        message: "Please select items before applying automatic discounts. Use the checkboxes in the table to select individual items.",
        tone: "warning"
      });
      return;
    }
  ...
  return (
    <Frame>
                      ...
                      <ButtonGroup>
                        ...
                        <ButtonGroup>
                          <Button 
                            ...
                            tone="success"
                          >
                            Apply Automatic Discounts {selectedItems.length > 0 ? `(${selectedItems.length})` : ''}
                          </Button>
                        ...
                        headings={[
                          <div style={{ display: 'flex', alignItems: 'center' }}>
                            <input 
                              ...
                              style={{ cursor: 'pointer', width: '16px', height: '16px' }}
                            />
                            <Text as="span" variant="bodySm" fontWeight="medium" color="subdued" style={{ marginLeft: '4px' }}>
      ...
      <Modal
        ...
        primaryAction={{
          content: `Apply Discounts to ${selectedItems.length} Item${selectedItems.length !== 1 ? 's' : ''}`,
      ...
      >
        <Modal.Section>
          <BlockStack gap="400">
            <Banner tone="info">
              <Text as="p">
                You have selected <strong>{selectedItems.length} item{selectedItems.length !== 1 ? 's' : ''}</strong> for automatic discounting.
                Discounts will be applied only to the selected items based on their expiration date.
              </Text>
            </Banner>
            
            <Text as="p">
              Automatic discounts will be applied to selected products based on their expiration date using the following rules:
            </Text>
